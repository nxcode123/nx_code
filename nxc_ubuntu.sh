...
NXC_LIB_LOCAL="$HOME/nxc_a#!/data/data/com.termux/files/usr/bin/bash
...
# Source file: nxc_ubuntu.sh
# CHANGELOG v1.2.8: 
# - Mengubah bahasa/teks loading agar lebih natural (tidak seperti AI/film sci-fi)
# - Menghapus limit daily update (Cocok untuk developer yang sering push bug fix)
# - Pengecekan update tetap dalam mode silent
# - Fix bug pemanggilan 'menu' di Ubuntu (menggunakan global binary file)
# ==============================================================================
...
# 1. Hushlogin setup
echo -e "${NEON_CYAN}[*] ${WHITE}Menyembunyikan teks sambutan awal (hushlogin)...${NC}"
...
if [ ! -d "$HOME/storage" ]; then
    ...
    echo -e "${NEON_CYAN}[*] ${WHITE}Akses penyimpanan HP ${NEON_GREEN}[✔ TERSEDIA]${NC}"
fi

# 3. Membersihkan Apt Locks, Status Macet, dan Cache Lama
echo -e "${NEON_CYAN}[*] ${WHITE}Membersihkan cache dan memperbaiki error paket dpkg...${NC}"
...
run_with_progress_bar "Meningkatkan (upgrade) paket dasar Termux" 20 "apt-get upgrade -y -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold'"
...
if ! command -v proot-distro &> /dev/null || ! command -v curl &> /dev/null; then
    ...
    echo -e "${NEON_CYAN}[*] ${WHITE}Modul Proot & Curl ${NEON_GREEN}[✔ TERSEDIA]${NC}"
fi

# 6. Install Ubuntu Rootfs via Proot-Distro (FIXED: Smart Skip & Auto-Repair)
...
if [ ! -d "$UBUNTU_ROOT" ] || [ ! -f "$UBUNTU_ROOT/etc/os-release" ]; then
    ...
    run_with_progress_bar "Menginstal sistem operasi Ubuntu (Harap tunggu)" 40 "proot-distro install ubuntu"
else
    echo -e "${NEON_CYAN}[*] ${WHITE}Sistem operasi Ubuntu ${NEON_GREEN}[✔ TERSEDIA]${NC}"
fi
...
echo -e "${NEON_CYAN}[*] ${WHITE}Menerapkan konfigurasi otomatis (.bashrc)...${NC}"
...
echo -e "\n${NEON_GREEN}[✔] SETUP TERMUX-UBUNTU SELESAI!${NC}"
lib.sh"
